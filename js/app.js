...
//Se guarda la cantidad de barritas que se van a ir pintando por cada barar
//Uso un arreglo, cada posiciòn pertenece a un elemento
//Comienza en -1 porque no tiene ninguna pintada al iniciarse
let contadores = [-1,-1,-1,-1,-1,-1];

//esta variable la uso de bandera para saber si ya ejecuto la animación
let entro = false;
...
function efectoHabilidades(){
    ...
    if(distancia_skills>=300 && entro==false){
        ...
        const intervalpowerbi = setInterval(function(){
            pintarBarra(powerbi, 16, 2, intervalpowerbi);
        },100);
        const intervalsql = setInterval(function(){
            pintarBarra(sql, 15, 3, intervalsql);
        },100);
        const intervalplatform = setInterval(function(){
            pintarBarra(platform, 16, 4, intervalplatform);
        },100);
